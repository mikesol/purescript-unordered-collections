...
"use strict";
var lowestBit = function (n) {
	return n & -n;
};

var mergeState = function (bit, thisnode, thisdata, thatnode, thatdata) {
...
var remove2insert1Mut = function (a, removeIndex, insertIndex, v1) {
...
class MapNode {
	constructor(datamap, nodemap, content) {
		this.datamap = datamap;
		this.nodemap = nodemap;
		this.content = content;
	}

	lookup(Nothing, Just, keyEquals, key, keyHash, shift) {
		...
		if ((this.nodemap & bit) !== 0) {
			return this.content[
				this.content.length - 1 - index(this.nodemap, bit)
			].lookup(Nothing, Just, keyEquals, key, keyHash, shift + 5);
		}
		return Nothing;
	}

	insertMut(keyEquals, hashFunction, key, keyHash, value, shift) {
		var bit = mask(keyHash, shift);
		var i = index(this.datamap, bit);
		if ((this.datamap & bit) !== 0) {
	...
	insert(keyEquals, hashFunction, key, keyHash, value, shift) {
		var bit = mask(keyHash, shift);
		var i = index(this.datamap, bit);
		if ((this.datamap & bit) !== 0) {
			var k = this.content[i * 2];
			if (keyEquals(k)(key))
				return new MapNode(
					this.datamap,
					this.nodemap,
					overwriteTwoElements(this.content, i * 2, key, value)
				);
			var newNode = binaryNode(
				k,
				hashFunction(k),
		...
		return new MapNode(
			this.datamap | bit,
			this.nodemap,
			insert2(this.content, i * 2, key, value)
		);
	}

	insertWith(keyEquals, hashFunction, f, key, keyHash, value, shift) {
		var bit = mask(keyHash, shift);
		var i = index(this.datamap, bit);
		if ((this.datamap & bit) !== 0) {
			var k = this.content[i * 2];
			if (keyEquals(k)(key))
				return new MapNode(
					this.datamap,
					this.nodemap,
					overwriteTwoElements(
						this.content,
						i * 2,
						key,
						f(this.content[i * 2 + 1])(value)
					)
				);
			var newNode = binaryNode(
				k,
	...
	delet(keyEquals, key, keyHash, shift) {
		var bit = mask(keyHash, shift);
		if ((this.datamap & bit) !== 0) {
			...
			if (keyEquals(this.content[dataIndex * 2])(key)) {
				...
				return new MapNode(
					this.datamap ^ bit,
					this.nodemap,
					remove2(this.content, dataIndex * 2)
				);
			}
			return this;
		}
		if ((this.nodemap & bit) !== 0) {
			...
			if (recRes.isSingleton()) {
				...
				return new MapNode(
					...
					insert2remove1(
						this.content,
						2 * index(this.datamap, bit),
						recRes.content[0],
						recRes.content[1],
						this.content.length - 1 - nodeIndex
					)
				);
			}
			return new MapNode(
	...
	toArrayBy(f, res) {
		...
		for (; i < this.content.length; i++) this.content[i].toArrayBy(f, res);
	}
	...
	unionWith(eq, hash, f, that, shift) {
		...
		while (skipmap !== 0) {
			...
			switch (
				mergeState(bit, this.nodemap, this.datamap, that.nodemap, that.datamap)
			) {
				case 1 /* NODE_NONE */:
				...
				case 5 /* NODE_NODE */:
					...
					nodemap |= bit;
					nodes.push(
						this.content[this.content.length - thisNodeIndex - 1].unionWith(
							eq,
							hash,
							f,
				...
				case 6 /* DATA_NODE */:
					...
					nodemap |= bit;
					nodes.push(
						that.content[that.content.length - thatNodeIndex - 1].insertWith(
							eq,
							hash,
							flippedF,
							k,
				...
				case 9 /* NODE_DATA */:
					...
					nodemap |= bit;
					nodes.push(
						this.content[this.content.length - thisNodeIndex - 1].insertWith(
							eq,
							hash,
							f,
							k,
				...
				case 10 /* DATA_DATA */:
					...
					thatDataIndex = index(that.datamap, bit);
					if (
						eq(this.content[thisDataIndex * 2])(that.content[thatDataIndex * 2])
					) {
						// equal, merge with f
						datamap |= bit;
						data.push(
							this.content[thisDataIndex * 2],
							f(this.content[thisDataIndex * 2 + 1])(
								that.content[thatDataIndex * 2 + 1]
							)
						);
					} else {
	...
	intersectionWith(Nothing, Just, eq, hash, f, that, shift) {
		if (this.constructor !== that.constructor)
		...
		while (skipmap !== 0) {
			...
			switch (
				...
				case 5 /* NODE_NODE */:
					thisNodeIndex = index(this.nodemap, bit);
					thatNodeIndex = index(that.nodemap, bit);
					var recRes = this.content[
						this.content.length - thisNodeIndex - 1
					].intersectionWith(
						Nothing,
						Just,
						eq,
				...
				case 6 /* DATA_NODE */:
					...
					var v = this.content[thisDataIndex * 2 + 1];
					var hk = hash(k);
					var res = that.content[
						that.content.length - thatNodeIndex - 1
					].lookup(Nothing, Just, eq, k, hk, shift + 5);
					if (res !== Nothing) {
						datamap |= bit;
	...
	filterWithKey(f) {
		...
		while (skipmap !== 0) {
			...
			if ((this.datamap & bit) !== 0) {
				...
				var nodeIndex = index(this.nodemap, bit);
				var node =
					this.content[this.content.length - nodeIndex - 1].filterWithKey(f);
...
class Collision {
	...
	insert(keyEquals, hashFunction, key, keyHash, value, shift) {
		var i = 0;
		for (; i < this.keys.length; i++) if (keyEquals(key)(this.keys[i])) break;
	...
	delet(keyEquals, key, keyHash, shift) {
		...
		if (this.keys.length === 2)
			return new MapNode(1 << (keyHash & 31), 0, [
				this.keys[1 - i],
				this.values[1 - i],
			]);
	...
	hash(vhash) {
		...
		for (var i = 0; i < this.values.length; i++) h += vhash(this.values[i]);
		return h;
	}
...
var mask = function (keyHash, shift) {
	return 1 << ((keyHash >>> shift) & 31);
};

var index = function (map, bit) {
...
var binaryNode = function (k1, kh1, v1, k2, kh2, v2, s) {
	...
	if (b1 !== b2)
		return new MapNode(
			(1 << b1) | (1 << b2),
			0,
			b1 >>> 0 < b2 >>> 0 ? [k1, v1, k2, v2] : [k2, v2, k1, v1]
		);

	return new MapNode(0, 1 << b1, [binaryNode(k1, kh1, v1, k2, kh2, v2, s + 5)]);
};

var overwriteTwoElements = function (a, index, v1, v2) {
	var res = a.slice();
	res[index] = v1;
	res[index + 1] = v2;
	return res;
...
var copyAndOverwriteOrExtend1 = function (a, index, v) {
	var res = a.slice();
	res[index] = v;
	return res;
};

var remove2insert1 = function (a, removeIndex, insertIndex, v1) {
...
var insert2 = function (a, index, v1, v2) {
...
var insert2remove1 = function (a, insertIndex, v1, v2, removeIndex) {
	var res = new Array(a.length + 1);
